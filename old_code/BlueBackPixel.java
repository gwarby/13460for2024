  ...
  public void runOpMode() 
  {
    ...
    if (opModeIsActive()) {
      ...
      if (lib.FindPropLocation() == "LEFT"){
         ...
         *
         ************************************************************************/
        lib.drive(17.5,-10,0,DRIVE_POWER);        // Fwd 15" toward spike marks

        lib.dropBottomPixel();

        lib.drive(-5, 0, 0, 0.2);       // back up: don't run over the pixel we just placed,
                                    // ...but also don't go back to far into frames
        lib.drive(0, 0, 90, DRIVE_POWER);      // CW 135 to face away from backdrop
        
        lib.drive(-12, -6, 0, DRIVE_POWER);       // BACK 24" toward backdrop
        lib.armraisewait(100, 0.3);         // raise arm 120 deg (all the way back/up for placing pixel on board)
        lib.armraisewait(20, 0.13);        // raise arm last 45 deg
         ...
         *
         ************************************************************************/
        lib.drive(28.0,-4,0,DRIVE_POWER);        // Fwd 15" toward spike marks
        lib.drive(0,0,-18,DRIVE_POWER);         // CCW 5 deg to be slightly offset from center of MIDDLE spike mark

        lib.dropBottomPixel();

        lib.drive(-3, 0, 0, 0.2);       // back up: don't run over the pixel we just placed,
                                    // ...but also don't go back to far into frames
        ...
        lib.drive(-17, 0, 0, DRIVE_POWER);       // FWD 25.5" toward backdrop
        lib.reverseFlipper();           // put flipper in rev pos for placing pixel on board
        sleep(500);
        ...
        lib.drive(-5 + yAdjustment, -3 + xAdjustment, 0, 0.2);       // REV last 5" to board
        sleep(500);
        lib.openClampWait();          // release pixel on board
        sleep(400);
        ...
        lib.groundTransitionFlipper();  // get arm in position to clear axle
        lib.armraise(-100, 0.41);        // bring the arm back down
        lib.drive(0, 26, 5, DRIVE_POWER); // drive right 18" to wall
        ...
        lib.drive(7, 0, 0, DRIVE_POWER);

        lib.dropBottomPixel();

        lib.drive(-8, 0, 0, DRIVE_POWER); // back up 2" to clear spike mark
        lib.drive(0, 0, 45, DRIVE_POWER);      // CW 40 to face away from backdrop
        sleep(50);
        lib.drive(-20, 0, 0, DRIVE_POWER);       // FWD 26" toward backdrop
        lib.armraise(100, 0.3);         // raise arm 120 deg (all the way back/up for placing pixel on board)
        //drive(0, 0, 180, DRIVE_POWER);      // CW 180 deg to back into backdrop
        lib.reverseFlipper();           // put flipper in rev pos for placing pixel on board
        lib.drive(0, -18, -3, DRIVE_POWER);       // left 4" along backdrop
        lib.armraise(30, 0.127);        // slow down to avoid tipping over
        ...
        lib.drive(-5.5 + yAdjustment, 0.3 + xAdjustment, 0, 0.2);       // REV last 5" to board
        sleep(500);
        lib.openClampWait();          // release pixel on board
        sleep(400);
        lib.drive(2 , 0, 0, 0.25);       // forward 2" from board
        lib.armraise(-100, 0.41);        // bring the arm back down
        lib.drive(0, 33.5, 3, DRIVE_POWER); // drive right 24" to wall
